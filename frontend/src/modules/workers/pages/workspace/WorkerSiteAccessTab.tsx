...
import { useWorkerSites } from "../../hooks/useWorkerSites";
import { Skeleton } from "@/components/ui/skeleton";

export function WorkerSiteAccessTab() {
  const { worker } = useOutletContext<{ worker?: WorkerDetail }>();
  const { data: sites, isLoading } = useWorkerSites(worker?.id);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {isLoading ? (
          ...
          sites.map((site) => (
            <div key={site.id} className="bg-card text-card-foreground shadow-sm border rounded-xl p-4 flex items-center justify-between group hover:border-primary transition-colors">
              ...
              <Link to={`/sites/${site.id}`} className="p-2 text-muted-foreground group-hover:text-primary transition-colors">
