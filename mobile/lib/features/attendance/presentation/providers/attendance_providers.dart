...
class AttendanceNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  Future<void> checkIn(String siteId, {double? lat, double? lng}) async {
    state = const AsyncValue.loading();
    try {
      await _repository.checkIn(siteId, lat: lat, lng: lng);
  ...
  Future<void> checkOut(String siteId, {double? lat, double? lng}) async {
    state = const AsyncValue.loading();
    try {
      await _repository.checkOut(siteId, lat: lat, lng: lng);
      state = const AsyncValue.data(null);
    } catch (e, st) {
      state = AsyncValue.error(e, st);
    }
